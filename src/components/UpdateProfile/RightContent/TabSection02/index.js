  ...
  return (
    <section id="02-privacy" className="tab-section">
      ...
      <div className="inner-container">
        <Password />
        <LastLogin 
          date={ !user.last_login ? <i className="fa fa-spinner fa-spin"></i> : dayjs(user.last_login).format(`DD - MM + YYYY`).replace('-', 'tháng').replace('+', 'năm') }
          time={ !user.last_login ? null : dayjs(user.last_login).format(`hh - mm + s --`).replace('-', 'giờ').replace('+', 'phút').replace('--', 'giây') }
        />
        <LastChangePassword
          date={ !user.last_change_password ? <i className="fa fa-spinner fa-spin"></i> : dayjs(user.last_change_password).format(`DD - MM + YYYY`).replace('-', 'tháng').replace('+', 'năm') }
